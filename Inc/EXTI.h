...
typedef struct
{
	...
	//Interrupt Request to NVIC
	uint8_t	  IVT_IRQ_Number;
}EXTI_GPIO_Mapping_t;

// this Prev code to make a constrains for user to write code according to specs

typedef struct
{
	EXTI_GPIO_Mapping_t  EXTI_Pin;  	// specifies External Interrupt GPIO Mapping (EXTI_line & PORTx & Pin_Number )
										//This Parameter must be set based on @ref EXTI_Define

	uint8_t IRQ_EN;						// Enable or Disable IRQ mask and also NVIC controller
										//This Parameter must be set based on @ref IRQ_Mask_enable


	uint8_t Trigger_Case; 				// Enable Rising or Failing Trigger or both
									    //This Parameter must be set based on @ref Trigger_Case


	void (* P_IRQ_CallBack)(void); 		//set the c function() which will be called once interrupt happen
...
#define EXTI_Trigger_FALLINGAndRISING		2
